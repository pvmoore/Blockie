...
final class TestScene6_bunny : SceneGenerator {
    ...
    World getWorld() {
        ...
        w.camera = new Camera3D(
            float3(xsize/2,ysize/2,5000), // position
            float3(xsize/2,500,xsize/2)     // focal point
        );
    ...
    void build(WorldEditor edit) {
        ...
        // floor
        edit.rectangle(int3(0,       0, 0),
                       int3(xsize-1, 1, zsize-1),
                       1,
                       V_ROCK1);
    ...
    void loadBunny(WorldEditor edit) {
        ...
        void blatSlice() {
            ...
            for(int x=0;x<512; x++)
            {
                ...
                if(value >= 45000) {
                    auto p = int3(300+x*N, 362*N-y*N, 300+(512*N-z*N));
